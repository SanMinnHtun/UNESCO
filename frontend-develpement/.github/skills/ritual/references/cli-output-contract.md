...
|---|---|---|
| **CLI / terminal** (terminal agents — Claude Code, Cursor, Codex in a built-in terminal) | Full two-line rail. `Ritual build` on line 1, `● Scope  ○ Discovery  ○ Recommendations  ○ Build brief  ○ Implementation (Your agent)` on line 2. | Terminal scrollback has no persistent UI — the rail IS the anchor. |
...
| `Implementation (Your agent)` | **Development-function builds ONLY.** Coding, branch/PR work, `sync_implementation`. Non-development builds (e.g. `create-launch-brief`) OMIT this stage — their rail has FOUR stages and ends at the deliverable. |
...
compactChip("implementation") => Ritual build · 5/5 Implementation (Your agent)   (dev only)
```
