...
1. **Static (skill-build / CI)** — the contract definition is linted in CI:
   `scripts/check-skill-voice.mjs` (authored-copy regexes) and
   `scripts/check-render-leaks.mjs` (forbidden-vocab corpus). These prove the
   skill copy itself never ships a leak and that the inline summaries stay in sync
   with this file.
...
3. **Deterministic — the CEILING (`render_gate`, where available)** — the
   `render_gate` MCP tool (#75 Slice 4) returns the EXACT canonical bytes for the
   ...
   identical bytes (the skill-side fallback copy is generated from the same server
   templates; a CI parity matrix pins them equal).
4. **Regression net (eval)** — ritual-skill-evals `no_render_leaks` scores every
   visible line of every build replay against this exact gate/status/nothing
   allowlist. It is the permanent proof that the contract holds, not a temporary
   crutch.
