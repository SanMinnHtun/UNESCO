...
<!-- skill-flow:prepare-build-entry: the entry collapses classify_work_item + workspace-resolution + create_draft_exploration into ONE prepare_build call (flowContractVersion 2→3). prepare_build classifies the job, AUTO-RESOLVES the workspace (no user pick), and creates/​resumes the draft. The Scope-entry gate stays one **[LITE AUTO — no pause; auto-pick the recommended default]** at Step 0.7; the Step 1 workspace-PICK + create-new pauses become one optional `change` escape; the whole Step 1.5 manual resume/suggest/overlap picker is removed (prepare_build's branch-resume is the only resume path). structuralBaseline 22→19 pauses / 26→25 steps. A JOB correction still re-calls classify_work_item (workspace stays sticky). -->
...
For narrow/mobile chat surfaces, use the **compact progress anchor** defined in `references/cli-output-contract.md` § Build progress anchor (the `Ritual build · 1/5 Scope` chip) instead of forcing the full five-stage rail to wrap. Same contract, different rendering. **When the Ritual plugin is attached (see *Plugin-attached rendering* below), the rail is plugin-owned — omit it entirely; the board renders live progress.**

### Plugin-attached rendering (load-bearing)
<!-- skill-flow:no-flow-change: plugin-attached rendering only collapses three RENDER surfaces to pointers — no MCP tool is added, removed, or reordered; the build tool sequence (flowContractVersion 2) is unchanged. -->
<!-- skill-options:no-gate-change: 2026-06-24 plugin-attached rendering — when a live Ritual plugin (board + drawer) owns a surface, the agent renders a one-line pointer instead of the full block, gated on a gitignored .ritual/plugin.local.json presence marker (same silent no-narrate read as .ritual/config.json). This adds NO pause gate, Step header, or option token — every pause gate and its tokens (incl. the universal `proceed`) are unchanged; only the verbose RENDER of three surfaces (build rail, discovery expert walk, recommendations review) collapses to a pointer. Absent/stale marker → full render, identical to today. Structural baseline unchanged: 22 pauses / 26 steps. -->
...
| **Discovery questions** (§ 7.3.1 landing + § 7.3.1b expert walk) | the landing block / Area-by-Area pick UI | `Discovery questions ready.` then `↳ Pick discovery questions in the Ritual panel, then reply \`proceed\`.` |
...
<!-- skill-options:no-gate-change: 2026-06-26 voice rule #1 hardening (from replay review) — also forbids rendering tool/field internals ("binding.action", "prepare_build", "resumed", "honoring binding…", "writing config.json") and step-transition narration ("now continuing to Scope"); reaffirms binding-write is silent. Copy/voice rule only — no pause gate, option token, or Step header added/removed/renamed (structural baseline 19 pauses / 25 steps unchanged). -->
<!-- skill-flow:no-flow-change: Voice rule #1 hardening + clarifying-question copy is copy/voice only; no MCP tool is added, removed, or reordered in the build flow's tool sequence. -->

**Copy rules (the calm-CLI-wizard contract — every gate obeys these):**
1. **Never print process/eval labels, platform-object lifecycle, tool/field internals, OR step-transition narration.** No `GATE N`, no `Step N`, no `Auto-decision: …`, no `LLM confidence`, no async-polling-contract talk. **Never name the exploration's internal lifecycle — no "draft", "draft started/created", "exploration created", "going LIVE", "locked".** **Never echo tool names, response fields, or directives — no "binding.action", "binding", "prepare_build", "resumed", "honoring binding…", "writing config.json".** **Never narrate the step transition — no "now continuing to Scope", "moving on to discovery", "next I'll…".** (Honoring the binding directive — writing `.ritual/config.json` — is SILENT: do it, never announce it.) The user is building something; the only vocabulary they ever see is the workflow (Scope → Discovery → Recommendations → Build brief → Implementation) and their workspace. The DRAFT→LIVE exploration and the `prepare_build` machinery are internal Ritual plumbing. The rail already shows where we are — use the rail or a compact header (`Ritual build · 1/5 Scope`), never both, and never a `GATE N` banner.
...
<!-- skill-flow:no-flow-change: voice-rule #8 hardening + de-imperative the "(silent)" step directions so the agent stops echoing the word "silently" into user copy (#71); copy/voice only — no tool added, removed, or reordered. -->
<!-- skill-options:no-gate-change: 2026-06-26 #71 voice — rule #8 lists render-intent/lifecycle/"silently" phrasings as forbidden, and the Scope-gate workspace lead line gets clearer copy ("Working in the {X} workspace · change to switch"); no pause gate, option token, or Step header is added, removed, or renamed (baseline 23 pauses / 25 steps unchanged). -->
<!-- skill-flow:no-flow-change: #75 render-contract extraction — rule #8 now points at the new references/render-contract.md (single source of truth for the gate/status/nothing allowlist); pointer/copy only, no tool added, removed, or reordered. -->
<!-- skill-options:no-gate-change: #75 render-contract.md is the canonical allowlist source; rule #8 + cli-output-contract.md gain a pointer to it. No pause gate, option token, or Step header is added, removed, or renamed (baseline 23 pauses / 25 steps unchanged). -->
...
|---|---|---|
| Scope-entry (Step 0.7) | `scope_entry` | `{ variant: '2a'\|'2b'\|'2c', deliverable, showImplementation, restatement, clarifyingQuestion? }` — from `prepare_build` / `classify_request` |
| Scope (§5.1) | `scope` | `{ deliverable, showImplementation, workspaceName, subProblems: [{n,title,blurb}], problemFrame }` — from `prepare_build` + `generate_considerations` + `generate_problem_statement` |
...
<!-- skill-flow:no-flow-change: #75 fail-closed-no-page — rule #8.2 field-sourcing clarification only (every render_gate field is required; showImplementation/workspaceName sourcing). render_gate stays optional + render-only; no tool added/removed/reordered in the required flow. -->
<!-- skill-options:no-gate-change: #75 fail-closed-no-page — render_gate field guidance + graceful-fallback wording only; no tracked pause gate, option token, or Step header is added, removed, or renamed (structural baseline unchanged). -->
...
- **`showImplementation`** — `true` only for **development-function** builds (the one whose rail carries the `Implementation (Your agent)` stage); `false` for product / marketing / prototyping. You already know the function from the Scope-entry classification (`prepare_build`) — thread the same boolean into *every* gate's fields, not just the first.
...
<!-- skill-options:no-gate-change: 2026-06-16 round-3 leak polish — turns voice rule #8 into a render allowlist (gate | approved status | nothing) with the verbatim status list; compacts the Scope-entry gate to one validation sentence; plainer workspace/overlap/rec-CTA copy; accept-recs screen keeps the next stage ○ not ●. Copy + behavioral rules only — no tracked pause gate, option token, or Step header is added, removed, or renamed (structural baseline unchanged: 22 pauses / 26 steps). -->

<!-- skill-options:no-gate-change: 2026-06-16 vocab cleanup — de-jargons internal labels the agent was parroting ("ordering barrier" → "before the Scope-entry gate is confirmed"; "silent recon" / "codebase recon" → "reading the codebase"). Wording only — no tracked pause gate, option token, or Step header is added, removed, or renamed (structural baseline unchanged: 22 pauses / 26 steps). -->

<!-- skill-options:no-gate-change: 2026-06-24 draft-on-Scope-entry — the build exploration is now created as a DRAFT right after classify (silently, before the Scope-entry gate) via create_draft_exploration so the plugin board lights up, and LOCKED to LIVE at Step 6 via lock_exploration_scope instead of create_exploration. The Scope-entry gate, Scope gate, and all pauses/options are unchanged — only WHICH tool runs WHEN (create→draft+lock) and silent plumbing move. No tracked pause gate, option token, or Step header is added, removed, or renamed (structural baseline unchanged: 22 pauses / 26 steps). -->
...
<!-- skill-flow:no-flow-change: rail drops the `Job` stage (it folds into Scope); the Step 0.7 gate, its user pause, and the prepare_build/classify_work_item tool sequence are all unchanged — render text + gate label only (flowContractVersion 3 unchanged). -->
...
<!-- skill-flow:prepare-build-entry: Step 0.7 first call is prepare_build (was classify_work_item); it returns the same classification fields PLUS workspace + binding + explorationId + resumed. Job correction re-calls classify_work_item (unchanged). No pause added/removed here. -->
...
2. **Render the validation prompt** (rail stage `Scope`). This gate is a plain-language VALIDATION of
   ...
   **2a — Confident match** (`isGenericFallback` is `false`): the classifier matched a specific job.
   ...
   ```text
   Ritual build
   ● Scope  ○ Discovery  ○ Recommendations  ○ {Deliverable}  ○ Implementation (Your agent)
...
<!-- skill-flow:prepare-build-entry: prepare_build (Step 0.7) already resolved the workspace + created the draft, so the DEFAULT path has NO workspace pause. Step 1 renders a one-line confirmation, honors the binding directive, and offers an optional user-invoked `change` escape (the ONLY pause here). The former workspace-PICK and create-new **[LITE AUTO — no pause; auto-pick the recommended default]**s are removed. -->
...
<!-- skill-options:no-gate-change: 2026-06-25 workspace→Scope stall structural fix — Step 1 no longer emits a standalone "Working in X" message (a complete-looking message read as a turn-end and stalled the headless flow ~50-67%). The workspace name + `change` affordance now ride on the Step 5 Scope-gate lead line, so the first render after the Scope-entry gate IS Scope. No pause gate or Step header added/removed/renamed (structural baseline 19 pauses / 25 steps unchanged); `change` is accepted at the existing Scope gate (no new gate). -->
<!-- skill-flow:no-flow-change: no load-bearing tool added, removed, or reordered (sequence + flowContractVersion 3 unchanged) — render-placement change only. -->
...
<!-- skill-flow:prepare-build-entry: no-arg build now just prompts for the ask, then runs prepare_build (Step 0.7). The former resume/suggest manual picker is gone — prepare_build's branch-resume is the only resume path. -->
...
> Scope-entry gate at Step 0.7 (server-side `classify_request`, user-confirmed). The lead-persona PICKER
...
Working in the {workspace.name} workspace · change to switch
...
<!-- skill-flow:no-flow-change: reuses get_exploration + get_considerations (both READS, not in the build tool sequence) + the existing §5.1a refine_problem_statement re-derive to commit the user's drawer-curated selection AND out-of-band problemStatement refinement — no MCP tool is added to, removed from, or reordered in the build sequence. -->
<!-- skill-options:no-gate-change: plugin-attached reconciliation only — the drawer is a SELECT surface for the EXISTING Scope gate; `use` and every option token are unchanged, no pause gate or Step header is added/removed/renamed (structural baseline unchanged). -->
...
<!-- skill-options:no-gate-change: #2a adds a canonical-state re-read (get_exploration) + reconcile before the EXISTING `use` lock — read-only, commits from canonical not chat memory; no pause gate, offered option, or Step header is added, removed, or renamed (structural baseline unchanged). -->
...
**Plugin-attached:** this landing is a plugin-owned surface (see *Plugin-attached rendering*). When the plugin is attached, do NOT render the rail or the landing block below — render EXACTLY these two lines (the rail is suppressed, so the first line is the readiness signal; `Discovery questions ready.` is the approved status — never `Recommendations ready.`, which belongs to Step 9):
...
<!-- skill-options:no-gate-change: the plugin-drawer discovery/rec behavior (curate in the drawer; on proceed read get_discovery_state selected:true and commit those) is folded into the Plugin-attached rendering notes at § 7.3.1b + Step 9 — no new gate option, pause, or command -->
<!-- skill-flow:no-flow-change: the plugin-drawer reconciliation only changes RENDER + reuses get_discovery_state (already a flow read) to commit the user's drawer-curated selection — no MCP tool is added, removed, or reordered in the build sequence (flowContractVersion 2 unchanged). -->

###### 7.3.1b — Expert mode: the Area walk (entered via `expert`)

**Plugin-attached:** this Area walk is a plugin-owned surface (see *Plugin-attached rendering*). When the plugin is attached, do NOT render the rail + Area UI below — render `↳ Pick discovery questions in the Ritual panel, then reply \`proceed\`.` and let the drawer drive selection (the questions are grouped-by-matter in the bottom-panel drawer; each toggle persists live to discovery state). **On `proceed`, the user has already curated — call `get_discovery_state`, take the questions with `selected: true` (grouped per matter), and commit exactly those via § 7.4's batch; do NOT re-run the in-chat Area walk.** The pick gate, its tokens, and `proceed` are unchanged.
...
**Plugin-attached:** this review is a plugin-owned surface (see *Plugin-attached rendering*). When the plugin is attached, do NOT render the compact landing / category walk below — render `↳ Review recommendations in the Ritual panel, then reply \`proceed\`.` and let the drawer show recs read-only. The non-blocking review still records `reviewedAt`/`reviewedBy` on `proceed`; the gate and its tokens are unchanged.
...
<!-- skill-flow:impl-work-list: 2026-06-26 — Step 11 becomes a per-slice LOOP. NEW 11.0.0 slices the brief into an ordered work-list (one PR per slice) and is the SINGLE entry to implementation by ANY path (continuous flow OR out-of-band brief arrival); NEW 11.6 loops back to it after each slice's PR, so the per-slice plan-mode handoff (11.0.5) fires once PER SLICE. Adds 2 user-pause gates (work-list confirm + loop-back; +4 raw markers — each gate has a header parenthetical + a body marker, matching 11.0.5); no new `#### Step` header (sub-steps use numbers, not the word "Step"). structuralBaseline 19→23 pauses / 25 steps unchanged. -->
<!-- skill-flow:no-flow-change: Step 11 adds the 11.0.0 work-list + 11.6 loop-back user-pause gates; no MCP tool is added, removed, or reordered in the build flow's tool sequence. -->
...
```text
Build brief is on disk. It's a multi-PR unit of work — here's the slice plan:

  1. {slice title} — {one line; RBs: RB-1, RB-4}
  2. {slice title} — {one line; RBs: RB-2}
  …

Each slice is its own branch → plan → PR, done one at a time, plan-first.
...
4. ~~`mcp__ritual__list_templates`~~ — **REMOVED 2026-05-21 (CLI 0.9.0+).** Step 2 is now server-side template resolution; the tool is no longer registered on the MCP surface. Do not call it. See Step 2 § "Rewritten 2026-05-21" for the rationale.
